...
| `CLUSTER INFO\|MYID\|SLOTS\|SHARDS\|NODES\|KEYSLOT key\|CELL lon lat` | introspection. With `LOCUS_CLUSTER_ENABLED` it reports real slot ownership and routes keys (`MOVED`/`CROSSSLOT`); off, `cluster_enabled:0`. `KEYSLOT` = CRC16 + `{hashtag}`; `CELL` returns the cell tag for a point (for `{cell}id` geo keys under cell-sharding) |
